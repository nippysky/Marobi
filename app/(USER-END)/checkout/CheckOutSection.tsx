...
/**
 * Checkout Section — FAANG-clean UX
 * - Users must pick a delivery option before paying.
 * - After payment succeeds and the order is created, we create a Shipbubble label:
 *     POST /api/shipping/shipbubble/labels
 *   Body = { requestToken, serviceCode, courierId, ... }
 */
...
import { useShipbubbleRates } from "@/lib/hooks/useShipbubbleRates";
...
export default function CheckoutSection({ user }: Props) {
  ...
  // Only show rates after essentials are present
  ...
  // Shipbubble rates
  const { loading: ratesLoading, rates: sbRates, error: ratesError, requestToken, boxUsed } = useShipbubbleRates({
    fullName,
    email,
  ...
  type SelectedShipRate = {
    requestToken: string;
    serviceCode: string;
    courierId: string;         // <- required by Shipbubble label API
    fee: number;
  ...
  // Ready to pay only when everything is valid AND user selected a courier
  ...
  // Helper for embedding Shipbubble meta in your order
  ...
  // Client helper to create the label AFTER we’ve created the order
    ...
    try {
      const resp = await fetch("/api/shipping/shipbubble/labels", {
        ...
        body: JSON.stringify({
          ...
          serviceCode: selectedShipRate.serviceCode,
          courierId: selectedShipRate.courierId,
          // Optional fields (kept out by default):
          // insuranceCode: "...",
          // isCodLabel: false,
        }),
      });
      ...
      // Success UX: show tracking link if present
      const tracking = json?.data?.tracking_url;
      if (tracking) {
        toast.success("Shipping label created. Tracking is available.");
      } else {
  ...
  // Payment success → create order → create label
  ...
  const handlePaystackSuccess = async (reference: any) => {
    try {
      ...
      // 🔵 Create Shipbubble label (non-blocking UX)
  ...
  const retryOrderCreation = async () => {
    ...
    try {
      ...
      // 🔵 Try label again after a successful retry
      await createShipbubbleLabelAfterOrder();
    } catch (err: any) {
                ...
                <FormField label="House address" htmlFor="houseAddress" span2>
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    ...
                    <em className="text-gray-700">
                      “{houseAddress || "63 Birnin Kebbi"}, {state || "Federal Capital Territory"},{" "}
                      {country?.name || "Nigeria"}”.
                    </em>{" "}
                    Keep it clear—street/house here; state & country are added automatically.
                  </p>
                </FormField>
              </div>
            </div>
              ...
              <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-md">
                ...
                {!formReadyForRates && (
                  <p className="text-sm text-gray-500">
                    Complete your name, email, phone, country, state and house address to see delivery options.
                  </p>
                )}

                {formReadyForRates && ratesLoading && <p className="text-sm text-gray-500">Fetching live rates…</p>}

                {formReadyForRates && !ratesLoading && Array.isArray(sbRates) && sbRates.length > 0 && (
                  <div className="grid gap-4">
                    {(sbRates as any[]).map((r: any, idx: number) => {
                            ...
                            <input
                              ...
                              onChange={() =>
                                setSelectedShipRate({
                                  ...
                                  courierId:
                                    r.courierId || r.courierCode || r.raw?.courier_id || "", // ensure we have the id
                                  fee: Number(r.fee) || 0,
                ...
                {formReadyForRates && !ratesLoading && Array.isArray(sbRates) && sbRates.length === 0 && (
                  <>
                    {ratesError ? <p className="text-xs text-red-600">Couldn’t fetch live rates: {ratesError}.</p> : null}
                    <p className="text-sm text-gray-500 mt-2">
                      Enter complete details to see delivery options. We’ll pick a suitable package automatically.
                    </p>
                  </>
                )}
              </div>
          ...
          <div className="space-y-6 flex flex-col min-h-0">
            ...
            <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-md flex flex-col">
              ...
              <div className="space-y-2 text-sm flex-1">
                ...
                <div className="flex justify-between"><span>Delivery Fee:</span><span>{formatAmount(shipFee, currency)}</span></div>

...
const STATUS_OPTIONS: AdminCustomerOrder["status"][] = [
  "Processing",
  "Shipped",
  "Delivered",
];
const CURRENCY_OPTIONS: AdminCustomerOrder["currency"][] = [
  "NGN",
  "USD",
  "EUR",
  "GBP",
...
export default function CustomerOrdersTable({ initialData }: Props) {
  const [data] = useState<AdminCustomerOrder[]>(initialData);
  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState<
    "All" | AdminCustomerOrder["status"]
  >("All");
  const [currencyFilter, setCurrencyFilter] = useState<
    "All" | AdminCustomerOrder["currency"]
  >("All");
  const [sorting, setSorting] = useState<any[]>([]);
  const [pagination, setPagination] = useState({ pageIndex: 0, pageSize: 10 });

  const [receiptOrder, setReceiptOrder] = useState<AdminCustomerOrder | null>(
    null
  );
  const [receiptOpen, setReceiptOpen] = useState(false);

  function openReceiptModal(order: AdminCustomerOrder) {
  ...
  function generateReceiptHtml(order: AdminCustomerOrder) {
    ...
    const itemsHtml = order.products
      ...
      .join("");
    return `<html><head><style>${receiptCSS}</style></head><body>
      <div class="header">
        <div>${now}</div>
  ...
  async function handlePrint(order: AdminCustomerOrder) {
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <span className={`px-2 py-0.5 rounded-full ${color}`}>{s}</span>
          );
        },
      },
      {

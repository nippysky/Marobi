...
const CURRENCY_OPTIONS: OrderRow["currency"][] = ["NGN", "USD", "EUR", "GBP"];

export default function OrderTable({ initialData }: Props) {
  const [data] = useState<OrderRow[]>(initialData); // not mutating orders here
  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState<
    "All" | OrderRow["status"]
  >("All");
  const [currencyFilter, setCurrencyFilter] = useState<
    "All" | OrderRow["currency"]
  >("All");
  ...
  function generateReceiptHtml(order: OrderRow) {
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <span className={`px-2 py-0.5 rounded-full text-sm ${color}`}>
              {s}
            </span>
          );
        },
                ...
                return (
                  <div className="px-2">
                    ...
                    <div className="flex justify-between">
                      <span>VAT (7.5%)</span>
                      <span>{sym}{vat.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Delivery</span>
                      <span>{sym}{deliveryCharge.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Weight</span>
